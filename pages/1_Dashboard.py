...
def kpi_card(col, css_cls, label, value, sub, btn_key, dialog_fn, *dialog_args):
    """
    Render one KPI card as a styled st.button.
    css_cls: 'kpi-default' | 'kpi-alert' | 'kpi-warn' | 'kpi-ok'
    The label is HTML with three spans for label/value/sub.
    """
    # Inject per-key CSS class on the wrapper div
    st.markdown(f"""
    <style>
    div[data-testid="stButton"]:has(button[data-testid="{btn_key}"]) {{
        /* Forward the CSS class — Streamlit doesn't let us set classes on wrappers,
           so we copy the border-left colour here directly */
    }}
    ...
    button[data-testid="{btn_key}"] p {{
        margin: 0 !important;
        text-align: left !important;
        white-space: normal !important;
    }}
    /* Streamlit puts all button text in a single <p> — we use spans to style */
    button[data-testid="{btn_key}"] .kpi-label-txt {{
        font-size: 0.62rem !important;
        font-weight: 600 !important;
        text-transform: uppercase !important;
        letter-spacing: 0.07em !important;
    ...
    button[data-testid="{btn_key}"] .kpi-value-txt {{
        font-size: 2.1rem !important;
        font-weight: 300 !important;
        line-height: 1 !important;
        font-family: 'IBM Plex Mono', monospace !important;
    ...
    button[data-testid="{btn_key}"]:hover .kpi-value-txt {{
        color: var(--teal) !important;
        text-decoration-style: solid !important;
        text-decoration-color: var(--teal) !important;
    }}
    button[data-testid="{btn_key}"] .kpi-sub-txt {{
        font-size: 0.63rem !important;
        color: var(--text-sm) !important;
        font-family: 'IBM Plex Sans', sans-serif !important;
        display: block !important;
    ...
    # The button label uses HTML spans for three text layers
    html_label = (
        f'<span class="kpi-label-txt">{label}</span>'
        f'<span class="kpi-value-txt">{value}</span>'
        f'<span class="kpi-sub-txt">{sub}</span>'
    )
    with col:
        if st.button(html_label, key=btn_key, use_container_width=True,
                     help=f"Click to view {label} detail"):
